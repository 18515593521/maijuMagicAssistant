...
Page({
  ...
  applyList: function (num){
    ...
    if (thisPage.data.selectArr2.length>0){
      ...
      console.log('添加的值' + JSON.stringify(thisPage.data.selectArr));
      console.log('提交的值1' + JSON.stringify( params));
    }

    wx.request({
      url: url + '/app/selectSellerCustomer',
      data: {           //请求参数      
        page: num,
  ...
  updateIsCustomer : function (){
    var thisPage = this;
    wx.request({
      url: url + '/app/updateIsCustomer/' + thisPage.data.customerId,
      header: {
        'content-type': 'application/json' // 默认值
      },
      method: 'GET',
